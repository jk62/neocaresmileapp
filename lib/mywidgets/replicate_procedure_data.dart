...
class ReplicateProcedureData extends StatefulWidget {
  final String? initialSourceClinicId;

  const ReplicateProcedureData({super.key, this.initialSourceClinicId});

  @override
  State<ReplicateProcedureData> createState() => _ReplicateProcedureDataState();
}

class _ReplicateProcedureDataState extends State<ReplicateProcedureData> {
  ...
  Future<void> _startReplication() async {
    ...
    try {
      await clinicSetupService.replicateProcedureData(
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('Procedure data replicated successfully.')),
        );
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Failed to replicate procedure data.')),
        );
      }
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Replicate Procedure Data'),
      ),
      body: Center(
        child: isLoading
